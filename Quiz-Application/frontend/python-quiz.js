...
async function fetchQuestions() {
    try {
        const response = await fetch('http://localhost:8080/api/python/all'); // API URL
        if (!response.ok) {
        ...
        document.getElementById('total-Question').textContent = totalQuestions; // Update the total questions in the HTML
        displayQuestion();
    } catch (error) {
